...
def get_data(): # Start recording
    ...
    try:
        while True:
            ...
            i=i+1
            if len(string2)>=139:
                box1rec_text.set('LED1: '+string2[20:24]+'  '+'PIR1: '+string2[26:30])
            if len(string2)>=139:
...
def show_conf(): # Show schedule configuration
    ...
    col11_2.grid(column=3,row=0,padx=5)
    col11_3.grid(column=4,row=0,padx=5)

    schedSep = ttk.Separator(tab11, orient=HORIZONTAL)
    schedSep.grid(column=0, row = 1, columnspan='6', sticky='we')
...
if __name__ == '__main__':
    ...
    configfilename = Label(text= 'Schedule').place(x=368, y=360)
